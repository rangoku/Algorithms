...
public class MD5 {
    ...
    public static byte[] computeMD5(byte[] msg) {
        ...
        int numBlocks = ((msgLenBytes + 8) >>> 6) + 1;
        int totalLen = numBlocks << 6;
        byte[] paddingBytes = new byte[totalLen - msgLenBytes];
        ...
        for (int i = 0; i < numBlocks; ++i) {
            paddingBytes[paddingBytes.length - 8 + i] = (byte)msgLenBits;
            msgLenBits >>>= 8;
        }
        ...
        for (int i = 0; i < numBlocks; ++i) {
            ...
            for (int j = 0; j < 64; ++j, ++index)
                buffer[j >>> 2] =
                        ((int)((index < msgLenBytes) ? msg[index] : paddingBytes[index - msgLenBytes]) << 24)
                        | (buffer[j >>> 2] >>> 8);
